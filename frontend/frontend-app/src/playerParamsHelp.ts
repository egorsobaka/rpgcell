...
export const playerParamsHelp: Record<string, ParamHelp> = {
  satiety: {
    name: 'Сытость',
    description: 'Текущий уровень сытости игрока. Определяет возможность движения.',
    source: 'Начальное значение: 255. Восстанавливается при использовании ресурсов из инвентаря (параметр food клетки).',
    calculation: 'Тратится при движении: moveCost = max(1, round(weight × 0.01 × (collectionPower - stamina))). Не может превышать weight.',
    effects: 'Если satiety < moveCost, игрок не может двигаться. Определяет выживаемость игрока.',
    initialValue: '255',
  },
  weight: {
    name: 'Вес / Максимальная сытость',
    description: 'Максимальное значение сытости и влияет на максимальный вес инвентаря.',
    source: 'Начальное значение: 255. Увеличивается при улучшении параметра "Вес".',
    calculation: 'При улучшении: weight = Math.round(weight × 1.1) (увеличение на 10%). Максимальный вес инвентаря: maxWeight = (weight / 2) + (weight / 2 × stamina / 10).',
    ...
    initialValue: '255',
    upgrade: 'Увеличивается на 10% при улучшении',
  },
  stamina: {
    name: 'Выносливость',
  ...
  experience: {
    ...
    calculation: 'Требуется для повышения уровня: requiredExperience = level × 255. При достижении уровня: experience -= requiredExperience, level += 1, availableUpgrades += 1.',
  ...
  level: {
    ...
    calculation: 'Повышается при накоплении опыта: requiredExperience = level × 255. При повышении уровня: level += 1, availableUpgrades += 1.',
  ...
  'experience-to-next': {
    ...
    calculation: 'requiredExperience = level × 255. Текущий опыт / требуемый опыт.',
